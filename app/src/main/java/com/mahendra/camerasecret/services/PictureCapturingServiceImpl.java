package com.mahendra.camerasecret.services;
...
public class PictureCapturingServiceImpl extends APictureCapturingService {
    ...
    private final ImageReader.OnImageAvailableListener onImageAvailableListener = imReader -> {
        ...
        public void onClosed(@NonNull CameraDevice camera) {
            ...
            if (!cameraIds.isEmpty()) {
                ...
                new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
                    ...
                    public void run() {
                        try {
                            final String[] cameraIdss = manager.getCameraIdList();
                            if (cameraIdss.length > 0) {
                                cameraIds.addAll(Arrays.asList(cameraIdss));
                                cameraIds.remove("0");
...
//                capturingListener.onDoneCapturingAllPhotos(picturesTaken);
    ...
    private void saveImageToDisk(final byte[] bytes) {
        ...
        final File file = new File(Environment.getExternalStorageDirectory() + "/" + Calendar.getInstance().getTime() + "_pic.jpg");
